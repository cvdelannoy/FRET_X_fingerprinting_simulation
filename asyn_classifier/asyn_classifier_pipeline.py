...
def plot_confusion_heatmap(df, true_var, pred_var, fold_var, var_names, out_dir):
    ...
    for cmap in ('Blues', 'Oranges', 'Greens', 'Purples', 'Reds'):
        fig, ax = plt.subplots(figsize=(5,5))
        sns.heatmap(data=heat_df, annot=annot_mat, fmt='s', cmap=cmap, annot_kws={'size': 7.5})
        plt.draw()
        ax.set_xlabel('Predicted', size=10)
        ax.set_ylabel('Truth', size=10)
        ax.set_xticks(ticks=ax.get_xticks(), labels=ax.get_xticklabels(), size=7.5, rotation=0)
        ax.set_yticks(ticks=ax.get_yticks(), labels=ax.get_yticklabels(), size=7.5)
