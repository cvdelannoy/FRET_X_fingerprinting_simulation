...
from helpers import parse_input_dir, parse_output_dir, get_FRET_distance

class ExperimentData(object):
    def __init__(self, txt_fn, exp_id, cutoff_low, cutoff_high, check_donor_only):
    ...
    def add_mol(self, mt):
        ...
        if len(event_dict) != 1: return
        self.fret_list.append([event_dict[edi]['position'] for edi in event_dict][0])


    def save(self, out_dir):
        with open(f'{out_dir}{self.exp_id}.pkl', 'w') as fh:
            fh.write('\n'.join([str(x) for x in self.fret_list]))
...
parser.add_argument('--cutoffs', type=float, nargs=2, default=[0.1, 1.0])
parser.add_argument('--out-fmt', type=str, choices=['pkl', 'txt'], default='pkl')
...
for txt_fn in txt_list:
    ...
    # exp_data = ExperimentData(txt_fn, exp_id, 0.93)
    exp_data = ExperimentData(txt_fn, exp_id, args.cutoffs[0], args.cutoffs[1], True)
    if args.out_fmt == 'pkl':
