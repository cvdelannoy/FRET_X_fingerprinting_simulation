...
import pandas as pd
import numpy as np
from CorrClassifier import CorrClassifier
...
for pdb_id in struct_dict:
    for cvi in struct_dict[pdb_id]['fingerprints']:
        if cvi == args.leave_out_fold:
            xt = {tn: struct_dict[pdb_id]['fingerprints'][cvi][tn]
            if np.sum(struct_dict[pdb_id]['fingerprints'][cvi][tn]) > 0 else None for tn in tagged_resn}
            if np.all([xt[resn] is None for resn in xt]): continue  # filter out invisible proteins
            X_test.append(xt)
