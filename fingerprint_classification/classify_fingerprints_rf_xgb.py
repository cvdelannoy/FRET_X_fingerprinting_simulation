...
with open(args.rf_pkl, 'rb') as fh: mod_txt, encoder = pickle.load(fh)
with open(args.fp_pkl, 'rb') as fh: struct_dict = pickle.load(fh)

tf = NamedTemporaryFile(suffix='.json', delete=True)
with open(tf.name, 'w') as fh: fh.write(mod_txt)
mod = XGBClassifier()
...
for pdb_id in struct_dict:
    for cvi in struct_dict[pdb_id]['fingerprints']:
        if cvi == args.leave_out_fold:
            xt = np.concatenate([struct_dict[pdb_id]['fingerprints'][cvi][resn] for resn in tagged_resn])
            if np.sum(xt) == 0: continue  # cannot see untagged structures
            X_test.append(np.concatenate([struct_dict[pdb_id]['fingerprints'][cvi][resn] for resn in tagged_resn]))
...
if len(rf_results_list):
    ...
    pred = encoder.inverse_transform(mod.predict(X_test))
